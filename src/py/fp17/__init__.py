...
def report(asg):
    ...
    for node in asg.nodes():
        if isinstance(node, EnumerationProxy):
            ...
            enumerations['wrapped'] += int(isinstance(node.boost_python_export, str))
    ...
    for node in asg.nodes():
        if isinstance(node, VariableProxy) and not isinstance(node, FieldProxy):
            variables['source'] += 1
            variables['wrapped'] += int(isinstance(node.boost_python_export, str))
    ...
    for node in asg.nodes():
        if isinstance(node, FieldProxy):
            fields['source'] += 1
            fields['wrapped'] += int(isinstance(node.boost_python_export, str))
    if fields['source'] > 0:
    ...
    for node in asg.nodes():
        if isinstance(node, FunctionProxy) and not isinstance(node, MethodProxy):
            functions['source'] += 1
            functions['wrapped'] += int(isinstance(node.boost_python_export, str))
    ...
    for node in asg.nodes():
        if isinstance(node, MethodProxy):
            methods['source'] += 1
            methods['wrapped'] += int(isinstance(node.boost_python_export, str))
    if enumerations['source'] > 0:
    ...
    for node in asg.nodes():
        if isinstance(node, EnumerationProxy):
            ...
            specializations['wrapped'] += int(isinstance(node.boost_python_export, str))
