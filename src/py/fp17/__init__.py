...
def report(asg):
    ...
    for node in asg.nodes():
        if isinstance(node, FieldProxy):
            fields['source'] += 1
            fields['wrapped'] += int(node.boost_python_export and not isinstance(node.boost_python_export, bool))
    if fields['source'] > 0:
    ...
    for node in asg.nodes():
        if isinstance(node, MethodProxy):
            methods['source'] += 1
            methods['wrapped'] += int(node.boost_python_export and not isinstance(node.boost_python_export, bool))
    if methods['source'] > 0:
